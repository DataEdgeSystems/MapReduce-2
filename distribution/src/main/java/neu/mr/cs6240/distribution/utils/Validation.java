package neu.mr.cs6240.distribution.utils;
...
public class Validation {
	...
	public static boolean isSane(String[] nextLine) {
		...
		if (nextLine.length != 110) return false;
		...
		try {
			...
			int dCRSArrTime = Integer.parseInt(CRSArrTime);
			int dCRSDepTime = Integer.parseInt(CRSDepTime);
			...
			if (Cancelled == 0) {
				if (ArrTime == null || ArrTime.isEmpty() || ArrTime.length() == 0 || DepTime == null
					...
					// ArrTime - DepTime - ActualElapsedTime -
					// timeZone
					// should be zero
					...
					// if ArrDelay < 0 then ArrDelayMinutes should
					// be zero
					if (dArrDelay < 0 && dArrDelayMinutes != 0) {
						throw new SanityFailedException("");
					}

					// if ArrDelayMinutes >= 15 then ArrDel15 should
					// be true
	...
	/* Calculate mins elapsed for Arrival and Departure */
